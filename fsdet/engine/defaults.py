...
__all__ = ["default_argument_parser", "default_setup", "DefaultPredictor", "DefaultTrainer", "Trainer", "BoVWTrainer"]
...
import time
from fsdet.modeling.roi_heads.roi_heads import PA_BoVW
from fsdet.evaluation import (
    COCOEvaluator,
    DatasetEvaluators,
    LVISEvaluator,
...
class BoVWTrainer(Trainer):
    def __init__(self, cfg):
        super().__init__(cfg)

        logger = logging.getLogger(__name__)
        logger.info("Using BoVW trainer......")

        self.bovw_model = PA_BoVW(cfg_path=cfg.MODEL.RESNET_BOVW,num_words=cfg.MODEL.NUM_WORDS)
        self.bovw_model.load_state_dict({k.replace("module.",""):v for k,v in torch.load(
            cfg.MODEL.BOVW_WEIGHT, map_location=torch.device('cpu')).items()}, strict=False)
        for p in self.bovw_model.parameters():
            p.requires_grad = False
        print("froze bovw_model parameters")
    ...
    def run_step(self):
        ...
        loss_dict = self.model(data, self.bovw_model)
