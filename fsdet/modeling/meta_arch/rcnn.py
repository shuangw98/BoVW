...
class GeneralizedRCNN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.MODEL.ROI_HEADS.FREEZE_FEAT:
            if hasattr(self.roi_heads, 'vocab_transform'):
                for p in self.roi_heads.vocab_transform.parameters():
                    p.requires_grad = False
                print("froze vocab_transform parameters")
            if hasattr(self.roi_heads, 'adaptive'):
                for p in self.roi_heads.adaptive.parameters():
                    p.requires_grad = False
                print("froze adaptive parameters")
            
            for p in self.roi_heads.box_head.parameters():
                p.requires_grad = False
            print('froze roi_box_head parameters')

            if hasattr(self.roi_heads, 'box_head_bow'):
                for p in self.roi_heads.box_head_bow.parameters():
                    p.requires_grad = False
                print("froze box_head_bow parameters")
            ...
            if cfg.MODEL.ROI_HEADS.UNFREEZE_FC2:
                ...
                if hasattr(self.roi_heads, 'box_head_bow'):
                    for p in self.roi_heads.box_head_bow.fc2.parameters():
                        p.requires_grad = True
                    print("unfreeze box_head_bow fc2 parameters")
            ...
            if cfg.MODEL.ROI_HEADS.UNFREEZE_FC1:
                ...
                if hasattr(self.roi_heads, 'box_head_bow'):
                    for p in self.roi_heads.box_head_bow.fc1.parameters():
                        p.requires_grad = True
                    print("unfreeze box_head_bow fc1 parameters")
                    
        print('-------- Using Roi Head: {}---------\n'.format(cfg.MODEL.ROI_HEADS.NAME))


    def forward(self, batched_inputs, bovw_model=None):
        """
        Args:
        ...
        # detector_losses includes Contrast Loss, 和业务层�?cls loss, and reg loss
        _, detector_losses = self.roi_heads(images, features, proposals, gt_instances, bovw_model)
